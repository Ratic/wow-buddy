...
var $WowheadPower = new function()
{
    ...
    this.init = function()
    {
        ...
        if(isRemote)
        {
            $WH.ae(head, $WH.ce('link', {type: 'text/css', href: (document.location.protocol != 'https:' ? 'http:' : document.location.protocol) + '//wowcss.zamimg.com/css/basic.css?8', rel: 'stylesheet'}));
        }
    ...
    function scanElement(t, e)
    {
        ...
        {
            if(k == 'buff' || k == 'sock' || k == 'map' || k == 'notip' || k == 'chal' || k == 'pvp' || k == 'noimage')
                params[k] = true;
            else if(k == 'rand' || k == 'ench' || k == 'tink' || k == 'lvl' || k == 'c' || k == 'diff' || k == 'diffnew' || k == 'upgd' || k == 'pwr' || k == 'q' || k == 'level')
                params[k] = parseInt(v);
            else if(k == 'gems' || k == 'pcs' || k == 'forg' || k == 'know' || k == 'cri' || k == 'bonus' || k == 'abil')
                params[k] = v.split(':');
        ...
        if(opt.applyto & 1)
        {
            ...
            {
                i0 = 1;
                m = t.href.match(/^https?:\/\/(.+?)?\.?(?:wowhead|thottbot)\.com(?:\:\d+)?\/\??(item|quest|spell|achievement|transmog-set|statistic|currency|npc|object|petability|building|follower|garrisonability|mission|hearthstone\/card)=([0-9\.]+)/);
                if (!m)
                {
            ...
            else
            {
                ...
                m = t.href.match(/()\/\??(item|quest|spell|achievement|transmog-set|statistic|currency|npc|object|petability|building|follower|garrisonability|mission|hearthstone\/card|card|cardback|deck|mechanic|adventure|boss)=([0-9\.]+)/);
        ...
        if(m == null && rel && (opt.applyto & 2))
        {
            ...
            i1 = 1;
            i2 = 2;
            m = rel.match(/(item|quest|spell|achievement|transmog-set|statistic|currency|npc|object|petability|building|follower|garrisonability|mission|hearthstone\/card|card|hsachievement|cardback|deck|mechanic|adventure|boss).?([0-9\.]+)/);
        ...
        if(params.bonus && params.bonus.length > 0)
        {
            ...
            if(i == 0)
                delete params.bonus;
            else if(i < params.bonus.length)
                params.bonus = params.bonus.slice(0, i);
        }

        // Clean up garrison abilities
        ...
        if(m) {
            ...
            if(/\bptr\d*\b/.test(domain))
                locale = 'ptr';
            else if(/\bbeta\d*\b/.test(domain) || /\bwod\b/.test(domain))
                locale = 'beta';
            ...
            var typeName = m[i1];
            if (isHearthhead && $WH.in_array(['achievement','quest'], typeName) >= 0) // these names shared with wowhead items, add hs prefix
            ...
            var timeout = function(t) {
                ...
                if (wowhead_tooltips.renamelinks) {
                    eval("t.innerHTML = t.innerHTML + '<span>' + data.name_" + LOCALES[locale] + " + '</span>';");
                }
    ...
    function request(type, id, locale, stealth, params)
    {
        ...
        for(var i in params)
        {
            if(i != 'rand' && i != 'ench' && i != 'tink' && i != 'gems' && i != 'sock' && i != 'diff' && i != 'diffnew' && i != 'upgd' && i != 'chal' && i != 'pvp' && i != 'lvl' && i != 'bonus' && i != 'q' && i != 'level' && i != 'abil')
                continue;
    ...
    function showTooltip(html, icon, map, spellData, html2, image, imageClass)
    {
        ...
        if (!tooltipInitiatedByUser) return;
        ...
        else if(currentParams != null)
        {
            ...
            if(currentParams.pcs && currentParams.pcs.length)
            {
                ...
                for(var i = 0, len = currentParams.pcs.length; i < len; ++i)
                {
                    ...
                    {
                        html = html.replace(m[0], '<span class="q8"><!--si' + currentParams.pcs[i] + '--><a href="/item=' + m[3] + '">' + (($WH.isset('g_items') && g_items[currentParams.pcs[i]]) ? g_items[currentParams.pcs[i]]['name_' + (!isNaN(parseInt(currentLocale)) ? LOCALES[currentLocale] : 'enus')] : m[4]) + '</a></span>');
                        ++n;
                    }
                }
        ...
        if ((!isRemote) && $WH.isset('g_user') && ('lists' in g_user) && $WH.isset('g_completion'))
        {
            ...
            if (complist)
                for (var c in g_user.lists)
                {
                    ...
                    var compId = /^-?\d+(?:\.\d+)?/.exec(currentId); // allow -123.45 and strip off everything after, like enchants, level, etc
        ...
        if ((!isRemote) && (currentType == TYPE_HEARTHSTONECARD) && $WH.isset('g_completion') && ('hearthstonecard' in g_completion) && LOOKUPS[currentType][0][currentId].collectible)
        {
            ...
            var comphtml = '<div class="q hearthhead-my-collection">' + window.LANG.lvmycollection + window.LANG.colon +
                '<span class="hearthhead-card-normal q' + (ccomp.normal < 2 ? ccomp.normal : '') + '">' + ccomp.normal + '</span> <span class="hearthhead-card-gold q' + (ccomp.golden < 2 ? ccomp.golden : '') + '">' + ccomp.golden + '</span>' +
                '</div>';
    ...
    {
        return id + (params.rand ? 'r' + params.rand : '') + (params.ench ? 'e' + params.ench : '') + (params.gems ? 'g' + params.gems.join(',') : '') + (params.sock ? 's' : '') + (params.upgd ? 'u' + params.upgd : '') + (params.chal ? 'c' : '') + (params.lvl ? 'lvl' + params.lvl : '') + (params.tink ? 'tink' + params.tink : '') + (params.pvp ? 'pvp' : '') + (params.bonus ? 'b' + params.bonus.join(',') : '') + (params.q ? 'q' + params.q : '') + (params.level ? 'level' + params.level : '') + (params.abil ? 'abil' + params.abil.join(',') : '');
    }
    ...
    this.register = function(type, id, locale, json)
    {
        ...
        if(currentType == type && id == currentId && currentLocale == locale)
        {
            ...
            showTooltip(
                arr[id][getTooltipField(locale)], 
                arr[id].icon, 
                arr[id].map, 
                arr[id][getSpellsField(locale)], 
